...
class FaviconManager {
    ...
    async loadFaviconWithRetry(maxRetries = 3, delay = 500) {
        for (let i = 0; i < maxRetries; i++) {
            ...
            console.log(`🔍 DEBUG attempt ${i + 1}: window.supabase =`, window.supabase);
            console.log(`🔍 DEBUG attempt ${i + 1}: typeof window.supabase =`, typeof window.supabase);
            if (window.supabase) {
                console.log(`🔍 DEBUG attempt ${i + 1}: window.supabase.createClient =`, typeof window.supabase.createClient);
                console.log(`🔍 DEBUG attempt ${i + 1}: window.supabase.from =`, typeof window.supabase.from);
            }
            ...
            if (supabaseClient && typeof supabaseClient.from === 'function') {
                ...
                console.log(`✅ Found working Supabase client on attempt ${i + 1}`);
