        ...
        for (let i = 0; i < maxRetries; i++) {
            ...
            if (window.supabase && typeof window.supabase.createClient === 'function') {
                try {
                    ...
                    console.log(`✅ Created Supabase client on attempt ${i + 1}`);
                    console.log(`🔍 Client test: this.supabase.from =`, typeof this.supabase.from);
                    
                    if (typeof this.supabase.from === 'function') {
                        await this.loadFavicon();
                        return;
                    } else {
                        console.warn(`⚠️ Created client but from() not available yet`);
                    }
                } catch (error) {
