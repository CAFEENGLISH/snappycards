...
class FaviconManager {
    ...
    async loadFavicon() {
        try {
            ...
            if (favicon && favicon.favicon_url) {
                console.log('🔍 Favicon URL found:', favicon.favicon_url);
                this.setFavicon(favicon.favicon_url, 'image/x-icon');
    ...
    async loadFaviconWithRetry(maxRetries = 3, delay = 500) {
        for (let i = 0; i < maxRetries; i++) {
            // Use existing Supabase client or create one if needed
